...
function WhyDoctorsDontPublish({ name }: { name?: string }) {
  return (
    <EmailShell>
...
function SixWeekStory({ name }: { name?: string }) {
  return (
    <EmailShell>
...
function BlankPageToDraft({ name }: { name?: string }) {
  return (
    <EmailShell>
...
function CVInTwelveMonths({ name }: { name?: string }) {
  return (
    <EmailShell>
...
export interface SequenceEmail {
  ...
  dayOffset: number
  subject: string
  build: (name?: string) => React.ReactElement
}

export const welcomeSequence: SequenceEmail[] = [
  { id: 'why-publish',   dayOffset: 1, subject: 'The #1 reason doctors never publish',        build: (name) => <WhyDoctorsDontPublish name={name} /> },
  { id: 'six-week-story', dayOffset: 3, subject: 'From blank page to published in 6 weeks',     build: (name) => <SixWeekStory name={name} /> },
  { id: 'product-demo',  dayOffset: 5, subject: 'Blank page → first draft in 10 minutes',      build: (name) => <BlankPageToDraft name={name} /> },
  { id: 'cv-offer',      dayOffset: 7, subject: 'Your CV, twelve months from now',             build: (name) => <CVInTwelveMonths name={name} /> },
]
